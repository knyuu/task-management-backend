...
class LocalizationController {
    ...
    init(body) {
        let dfd = q.defer();
        let dfdAr = [];
        // Guard against missing session or language property
        const currentKey = body.session && body.session.language ? body.session.language.current : undefined;
        dfdAr.push(LanguageProvider.getCurrentLanguage(body._service[0].dbname_prefix, currentKey));
