...
class RingBellItemService {
    ...
    insert(dbname_prefix, username, action, params, to_username, seen, from_action, notify_time, to_students = []) {
        ...
        // 1. First insert into MongoDB as usual
        MongoDBProvider.insert_onManagement(dbname_prefix, "user", username,
            ...
            .then(function(result) {
                ...
                // 2. Fetch recipient user details to check mute status
                MongoDBProvider.load_onManagement(dbname_prefix, "user", { username: { $in: toUsernameArray } }, undefined, undefined, undefined, { username: 1, is_muted: 1, mute_until: 1 })
                    .then(function(users) {
                        const userMuteMap = {};
                        if (users) {
                            users.forEach(u => {
                                let muted = u.is_muted === true;
                                if (!muted && u.mute_until) {
                                    muted = new Date(u.mute_until) > new Date();
                                }
                                userMuteMap[u.username] = muted;
                            });
                        }

                        // 3. Send notifications to users via socket if not muted
                        for (let i = 0; i < toUsernameArray.length; i++) {
                            const targetUsername = toUsernameArray[i];
                            const isMuted = userMuteMap[targetUsername] || false;

                            if (targetUsername && seenArray.indexOf(targetUsername) === -1) {
                                if (isMuted) {
                                    console.log(`[RingBellItemService] 🔇 User ${targetUsername} is muted, skipping socket notification`);
                                    continue;
                                }
                    ...
                    .catch(function(err) {
                        console.error('[RingBellItemService] ⚠️ Error fetching user mute status, proceeding with default notification behavior:', err);
                        // Fallback: continue with original notification logic if user lookup fails
                        for (let i = 0; i < toUsernameArray.length; i++) {
                            const targetUsername = toUsernameArray[i];
                            if (targetUsername && seenArray.indexOf(targetUsername) === -1) {
                                SocketProvider.IOEmitToRoom(targetUsername, "new_ringbell_item", {
