...
class UserController {
    ...
    updateProfile(body) {
        ...
        const avatar_color = body.data?.avatar_color;
        const password = body.data?.password;
        const currentPassword = body.data?.currentPassword;
        const is_muted = body.data?.is_muted;
        const mute_until = body.data?.mute_until;
        const theme = body.data?.theme;
        const default_launch_tab = body.data?.default_launch_tab;
        const default_dynamic_tab = body.data?.default_dynamic_tab;
        const is_sound_enabled = body.data?.is_sound_enabled;
        const language = body.data?.language;
        const notify_assigned = body.data?.notify_assigned;
        const notify_comment = body.data?.notify_comment;
        const notify_deadline = body.data?.notify_deadline;

        if (password && currentPassword) {
            const encryptedCurrent = AuthenticationProvider.encrypt_oneDirection_lv1(currentPassword);
            const encryptedNew = AuthenticationProvider.encrypt_oneDirection_lv1(password);
            
            UserService.checkExists(dbname_prefix, username, encryptedCurrent).then(function(users) {
                if (users && users[0]) {
                    UserService.updateProfile(dbname_prefix, username, title, encryptedNew, avatar_color, is_muted, mute_until, theme, default_launch_tab, default_dynamic_tab, is_sound_enabled, language, notify_assigned, notify_comment, notify_deadline)
                        .then((res) => dfd.resolve(res), (err) => dfd.reject(err));
                } else {
                    dfd.reject({ path: "UserController.updateProfile.InvalidPassword", mes: "InvalidPassword" });
                }
            }).catch(err => dfd.reject(err));
        } else {
            UserService.updateProfile(dbname_prefix, username, title, undefined, avatar_color, is_muted, mute_until, theme, default_launch_tab, default_dynamic_tab, is_sound_enabled, language, notify_assigned, notify_comment, notify_deadline)
                .then((res) => dfd.resolve(res), (err) => dfd.reject(err));
        }
    ...
    insert(body) {
        ...
        UserService.countUser(body._service[0].dbname_prefix)
            .then((totalUser) => {
                // check user limit and reject
                return UserService.checkUserLimit_onBusiness(body._service[0].domain, totalUser);
            })
            .then((result) => {
    ...
    registerPublic(body) {
        console.log('DEBUG: UserController.registerPublic received request');
        ...
        const dbname_prefix = body._service && body._service[0] ? body._service[0].dbname_prefix : ''; 
        console.log('DEBUG: Using dbname_prefix:', dbname_prefix);
        ...
        UserService.registerPublic(
            ...
            console.log('DEBUG: UserService.registerPublic success');
            dfd.resolve(true);
        }).catch((err) => {
            console.error('DEBUG: UserService.registerPublic error:', err);
            dfd.reject({
    ...
    loginWithGoogle(body) {
        let dfd = q.defer();
        const dbname_prefix = body._service && body._service[0] ? body._service[0].dbname_prefix : "";
        const token = body.token; // Firebase ID Token (JWT) from Mobile

        UserService.loginWithGoogle(dbname_prefix, token).then(function (result) {
    ...
    /**
     * Login or register with Google (Web - OAuth Code flow)
     */
    loginWithGoogleWeb(body) {
        let dfd = q.defer();
        const dbname_prefix = body._service && body._service[0] ? body._service[0].dbname_prefix : "";
        const code = body.code; // OAuth Authorization Code from Web redirect

        UserService.loginWithGoogleWeb(dbname_prefix, code).then(function (result) {
            const user = result.user;
    ...
    resetPasswordWithCode(body) {
        ...
        UserService.resetPasswordWithCode(dbname_prefix, email, code, newPassword).then(function (result) {
            dfd.resolve(result);
        }, function (err) {
            dfd.reject(err);
        });
