...
validation.loginWithGoogle = function (req, res, next) {
    const schema_body = {
        token: Joi.string().required() // Firebase ID Token (JWT) from Mobile
    };
    ValidationProvider.createMiddleware(schema_body, req, res, next);
}

validation.loginWithGoogleWeb = function (req, res, next) {
    const schema_body = {
        code: Joi.string().required() // OAuth authorization code from Web redirect
    };
    ValidationProvider.createMiddleware(schema_body, req, res, next);
}
