...
const { FirebaseAuthProvider } = require('../../../shared/firebase-auth/firebase-auth.provider');


const nameLib = 'avatar';
const EmailVerificationProvider = require('../../../shared/email-verification/email-verification.provider');
...
class UserService {
    ...
    loginWithGoogle(dbname_prefix, token) {
        let dfd = q.defer();
        
        console.log('DEBUG: UserService.loginWithGoogle (Firebase) started');
        
        FirebaseAuthProvider.verifyIdToken(token).then(function (decodedToken) {
            const email = (decodedToken.email || "").toLowerCase();

            if (!email) {
                dfd.reject({ mes: "Firebase token did not return email" });
                return;
            }
            ...
            console.error("UserService.loginWithGoogle (Firebase) error:", err);
            dfd.reject({ mes: err && err.message ? err.message : "Firebase identity verification failed" });
        });

        return dfd.promise;
    }

    loginWithGoogleWeb(dbname_prefix, code) {
        let dfd = q.defer();

        GoogleAuthProvider.authenticate(code).then(function (authResult) {
            const googleUser = authResult.user;
            const email = googleUser.email ? googleUser.email.toLowerCase() : null;

            if (!email) {
                dfd.reject({ mes: "Google did not return an email address" });
                return;
            }
                ...
                .then(function (users) {
                    if (users && users[0]) {
                        return MongoDBProvider.update_onManagement(
                            ...
                            .then(function (user) {
                                dfd.resolve({ user, isNewUser: false });
                            });
                    }
                    ...
                    return MongoDBProvider.insert_onOffice(dbname_prefix, "employee", email, {
                        fullname: title, department: "Public", competence: "Staff", update_mission_general: true
                    }).then(function (e) {
                        const employeeId = e && e.ops && e.ops[0] ? e.ops[0]._id.toString() : null;
                        return MongoDBProvider.insert_onManagement(dbname_prefix, "user", email, {
                            username: email, email, title,
                            title_search: removeUnicode(title),
                            password, language: { current: "vi-VN" },
                            isactive: true, rule: [{ rule: "Authorized" }], role: [],
                            employee: employeeId, department: "Public",
                            email_verified: true, avatar_url: googleUser.picture || null
                        });
                    }).then(function () {
                        return MongoDBProvider.getOne_onManagement(dbname_prefix, "user", { username: { $eq: email } });
                    }).then(function (user) {
                        dfd.resolve({ user, isNewUser: true });
                    });
                })
                .catch(function (err) { dfd.reject(err); });
        }, function (err) {
            console.error("UserService.loginWithGoogleWeb error:", err);
            dfd.reject({ mes: err && err.message ? err.message : "Google authentication failed" });
        });
    ...
    updateProfile(dbname_prefix, username, title, password, avatar_color, is_muted, mute_until, theme, default_launch_tab, default_dynamic_tab, is_sound_enabled, language, notify_assigned, notify_comment, notify_deadline) {
        const updateObj = {};
        if (title) {
            updateObj.title = title;
            updateObj.title_search = removeUnicode(title);
        }
        if (password) {
            updateObj.password = password;
        }
        if (avatar_color) {
            updateObj.avatar_color = avatar_color;
        }
        if (is_muted !== undefined) {
            updateObj.is_muted = is_muted;
        }
        if (mute_until !== undefined) {
            updateObj.mute_until = mute_until;
        }
        if (theme) {
            updateObj.theme = theme;
        }
        if (default_launch_tab !== undefined) {
            updateObj.default_launch_tab = default_launch_tab;
        }
        if (default_dynamic_tab) {
            updateObj.default_dynamic_tab = default_dynamic_tab;
        }
        if (is_sound_enabled !== undefined) {
            updateObj.is_sound_enabled = is_sound_enabled;
        }
        if (language) {
            updateObj.language = language;
        }
        if (notify_assigned !== undefined) {
            updateObj.notify_assigned = notify_assigned;
        }
        if (notify_comment !== undefined) {
            updateObj.notify_comment = notify_comment;
        }
        if (notify_deadline !== undefined) {
            updateObj.notify_deadline = notify_deadline;
        }
    ...
    checkUserLimit_onBusiness(domain, totalUser) {
        const dfd = q.defer();

        MongoDBProvider.load(
            undefined,
            mongoDBSettings.connectName.host.business,
            "service",
            { domain: domain }
        )
            .then((data) => {
                const userLimit = data[0].limit_user;

                if (totalUser > userLimit) {
                    dfd.reject({ mes: "User has reached maximum limit" });
                }
                else {
                    dfd.resolve(true);
                }
    ...
    sendVerificationCode(dbname_prefix, email) {
        return EmailVerificationProvider.sendVerificationCode(email);
    }

    verifyCode(dbname_prefix, email, code) {
        const dfd = q.defer();
        EmailVerificationProvider.verifyCode(email, code).then(() => {
            return this.loadDetails(dbname_prefix, email);
        }).then((user) => {
            dfd.resolve({ user, message: "CodeVerifiedSuccessfully" });
        }).catch((err) => {
            dfd.reject(err);
        });
        return dfd.promise;
    ...
    resetPasswordWithCode(dbname_prefix, email, code, newPassword) {
        const dfd = q.defer();
        EmailVerificationProvider.verifyCode(email, code).then(() => {
            const encryptedPassword = AuthenticationProvider.encrypt_oneDirection_lv1(newPassword);
            let d = new Date();
            return MongoDBProvider.update_onManagement(dbname_prefix, "user", email,
                { username: { $eq: email } },
                {
                    $set: { password: encryptedPassword },
                    $push: { events: { username: email, action: "ResetPasswordWithCode", time: d.getTime() } }
                });
        }).then(() => {
            dfd.resolve(true);
        }).catch((err) => {
    ...
    deleteAccount(dbname_prefix, username) {
        const dfd = q.defer();
        const self = this;
        this.loadDetails(dbname_prefix, username).then(function (user) {
            if (!user) {
                dfd.resolve(true); 
                return;
            }
            ...
            dfdAr.push(MongoDBProvider.delete_onManagement(dbname_prefix, "user", username, { username: { $eq: username } }));

            // 2. Delete from employee collection if exists
            if (user.employee) {
                dfdAr.push(MongoDBProvider.delete_onOffice(dbname_prefix, "employee", username, { _id: { $eq: new require('mongodb').ObjectID(user.employee) } }));
            }
