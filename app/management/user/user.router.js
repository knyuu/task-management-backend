...
router.post('/login', MultiTenant.match(), validation.login, Router.trycatchFunction("post/management/user/login", function (req, res) {
    return function () {
        const dbname_prefix = req.body._service && req.body._service[0] ? req.body._service[0].dbname_prefix : "";
        UserController.login(req.body).then(function (data) {
            // Resolve avatar URL before sending response
            const user = data.data;
            if (user && user.avatar && user.avatar.name && user.avatar.nameLib) {
                ...
                // Handle Google avatar_url (no file reference)
...
// Web Google OAuth route (receives OAuth authorization code)
router.post('/google/login-web', MultiTenant.match(), validation.loginWithGoogleWeb, Router.trycatchFunction("post/management/user/google/login-web", function (req, res) {
    return function () {
        UserController.loginWithGoogleWeb(req.body).then(function (data) {
            res.send(data);
            res.end();
            data = undefined;
            res = undefined;
            ...
            res.status(statusHTTP.authorized);
            Router.LogAndMessage(res, "post/management/user/google/login-web", err);
            res.end();
            err = undefined;
            res = undefined;
            req = undefined;
...
router.post('/resetPasswordWithCode', MultiTenant.match(), Router.trycatchFunction("post/management/user/resetPasswordWithCode", function (req, res) {
    return function () {
        UserController.resetPasswordWithCode(req.body).then(function (data) {
            res.send(data);
            res.end();
            data = undefined;
            res = undefined;
            ...
            res.status(statusHTTP.internalServer);
            Router.LogAndMessage(res, "post/management/user/resetPasswordWithCode", err);
            res.end();
            err = undefined;
            res = undefined;
            req = undefined;
