        ...
        return function (req, res, next) {
            ...
            SessionProvider.verify(req, res).then(function (data) {
                ...
                if (typeof rule === "string") {
                    if (!req.body.session || !Array.isArray(req.body.session.rule) || req.body.session.rule.filter(e => e.rule === "*" || e.rule === rule).length == 0) {
                ...
                if (typeof rule === "object") {
                    let check = false;
                    for (var i in rule) {
                        if (req.body.session && Array.isArray(req.body.session.rule) && req.body.session.rule.filter(e => e.rule === "*" || e.rule === rule[i]).length > 0) {
                            check = true;
                            next();
                            break;
                        }
